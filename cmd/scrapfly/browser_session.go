...
func newBrowserClickCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "click <locator>",
		Short: "Click an element — locator is an AXTree ref (e3), CSS selector, XPath, or AX node id",
		Long: `Locator resolution order:
  1. "e\d+" → AXTree ref from the last snapshot (uses Input.dispatchMouseEvent).
  2. --selector-type=xpath <expr>          → Antibot selector {type:xpath}
  3. --selector-type=axNodeId <ax_node_id> → Antibot selector {type:axNodeId}
  4. anything else                         → Antibot selector {type:css} (default, human-like click)`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			req := sessiond.Request{Action: "click", Ref: args[0]}
			if selType != "" && selType != "css" {
				req.Selector = args[0]
...
func newBrowserFillCmd(flags *rootFlags) *cobra.Command {
  ...
  - CSS/XPath/axNodeId via --selector-type goes through Antibot.fill on Scrapfly browsers.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			req := sessiond.Request{Action: "fill", Ref: args[0], Text: args[1], Clear: clear, WPM: wpm}
			if selType != "" && selType != "css" {
				req.Selector = args[0]
				req.SelectorType = selType
...
func newBrowserScrollCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Scroll up|down|left|right",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runAction(flags, "browser.scroll", sessiond.Request{
				Action: "scroll", Direction: args[0], Amount: amount, Ref: ref,
			})
		},
	}
	cmd.Flags().Float64Var(&amount, "amount", 500, "scroll distance in px")
	cmd.Flags().StringVar(&ref, "ref", "", "anchor element ref (optional)")
	return cmd
}
