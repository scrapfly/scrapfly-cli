...
func buildAgentTools(answerSchema json.RawMessage) []llm.Tool {
	...
	tools = append(tools,
		...
		llm.Tool{
			Name:        "find_selector",
			Description: "Given a natural-language description of an element, produce a verified CSS selector. Uses the currently-loaded page. Useful when you want a stable selector for later automation instead of relying on ephemeral refs. Returns {selector, match_count, sample_text}.",
			InputSchema: mustSchema(`{"type":"object","properties":{"description":{"type":"string","description":"Natural-language description of the element"},"want_text":{"type":"string","description":"Optional substring the selector's match must contain (case-insensitive)"}},"required":["description"]}`),
		},
		llm.Tool{
			Name:        "done",
...
func runAgentTool(rt *agentRuntime, tc llm.ToolCall) (string, error) {
	...
	case "find_selector":
		...
		if err := json.Unmarshal(tc.Input, &fsargs); err != nil {
			return "", fmt.Errorf("invalid find_selector args: %w", err)
		}
		// Pull the page HTML straight from the live session so the selector
		// is verified against what the agent is actually looking at.
		html, err := rt.sess.Eval(ctx, "document.documentElement.outerHTML")
		if err != nil {
			return "", fmt.Errorf("find_selector: read page html: %w", err)
		}
		htmlStr, _ := html.(string)
		if htmlStr == "" {
			return "", fmt.Errorf("find_selector: empty page html")
		}
		// No root flags available here — selector finder builds its own LLM
		// provider via autodetect, same as the top-level command.
		result, err := runSelectorFinder(ctx, &rootFlags{}, fsargs.Description, htmlStr, fsargs.WantText, 4, "", "")
		if err != nil {
			return "", err
		}
